...
{
    public class OwnBookingForShipGetAll
    {
        ...
        public class Handler : IRequestHandler<Query, Result<List<BookingPreviewDataDto>>>
        {
            ...
            {
                if (!_context.Ships.Any(x => x.Id.Equals(request.Id)))
                {
                    return Result<List<BookingPreviewDataDto>>.Failure("Fail, ship does not exist.");
                }
                ...
                var bookings = user.Ships
                    .Where(x => x.Id.Equals(request.Id))
                    .SelectMany(x => x.Bookings).ToList();
